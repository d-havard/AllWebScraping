...
namespace ExcelLocalBiblioC
{
    ...
    public class ExcelStructure
    {
        ...
        public WorkSheet VerificationIfFileExist(string nomSheet)
        {
            if (!File.Exists(OriginalFilePath))
            {
                workBook = WorkBook.Create(ExcelFileFormat.XLSX);
                workSheet = workBook.CreateWorkSheet(nomSheet);
            }
            else
            {
                workBook = WorkBook.LoadExcel(OriginalFilePath);
                workSheet = workBook.GetWorkSheet(nomSheet);
                if (workSheet == null)
                {
                    workSheet = workBook.CreateWorkSheet(nomSheet);
                }
        ...
        public void CreateStructure(string OriginalPath, string nomSheet)
        {
            ...
            if (workSheet["C7"].IsEmpty)
            {
                ...
                if (dayMonthInt != actualDayMonthInt)
                {
                    int column = 2;
                    for (int i = 0; i < actualDayMonthInt - 1; i++)
                    {
                        workSheet.Rows[6].Columns[column].Value = firstDayOfMonth.AddDays(i);
            ...
            if (workSheet["C103"].IsEmpty)
            {
                DateTime date = DateTime.Now;
                string[] actualMonth = date.ToString().Split('/');
                var firstMonth = new DateTime(date.Year, 1, 1);
                var lastMonth = firstMonth.AddMonths(11);
                string[] firstMonth1 = firstMonth.ToString().Split('/');
                string[] lastMonth1 = lastMonth.ToString().Split('/');
                int firstMonthInt = Int32.Parse(firstMonth1[1]);
                int lastMonthInt = Int32.Parse(lastMonth1[1]);
                int actualMonthInt = Int32.Parse(actualMonth[1]);
                Console.WriteLine(firstMonthInt);
                Console.WriteLine(lastMonthInt);
                if (firstMonthInt != actualMonthInt)
                {
                    int column = 2;
                    for (int i = 0; i < actualMonthInt - 1; i++)
                    {
                        workSheet.Rows[102].Columns[column].Value = firstMonth.AddMonths(i);
                        Console.WriteLine(firstMonth.AddDays(i));
                        column++;
                    }
                }
            }
        ...
        {
            workBook.SaveAs(OriginalFilePath);
        }
    }
}